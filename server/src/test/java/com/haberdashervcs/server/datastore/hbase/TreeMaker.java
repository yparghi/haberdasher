...
final class TreeMaker {
    ...
    private void actuallyWriteFolder(String folderPathStr, Set<AddedFileEntry> filesThisFolder) throws IOException {
        ...
        for (AddedFileEntry added : filesThisFolder) {
            ...
            if (mergeLock == null) {
                folder = FolderListing.withoutMergeLock(entries, folderPathStr, this.commitId);
            } else {
                folder = FolderListing.withMergeLock(entries, folderPathStr, this.commitId, mergeLock.getId());
            }
            ...
            helper.putFile(
                    ...
                    FileEntry.forNewContents(added.contents.getBytes(StandardCharsets.UTF_8)));
        }
    }
}
